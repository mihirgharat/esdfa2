//const api_url = "https://esd3tcet.herokuapp.com/"
const api_url = "https://esd3tcet.herokuapp.com/user"
...
function loadData(records = []) {
	...
	for(let i=0; i<records.length; i++) {
		...
		table_data += `<td>${records[i].crime}</td>`;
		table_data += `<td>`;
...
function getDataById(id) {
	...
	.then((data) => { 
		...
		document.getElementById("crime").value = data.crime;
	})
}
...
function postData() {
	...
	data = {name: name, age: age, city: city, crime: crime};
...
function putData() {
	...
	data = {_id: _id, name: name, age: age, city: city, crime: crime};
